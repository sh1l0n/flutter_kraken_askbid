...
class CommonPanControlBehaviourInitialState {
  const CommonPanControlBehaviourInitialState(this.scale, this.dx, this.width);
  final double scale;
  final double dx;
  final int width;
}

class CommonPanControlBehaviour<D> implements ChartBehavior<D> {
  CommonPanControlBehaviour(this.initialState) {
  ...
  PanningUpdateallback _panningUpdateCallback;
  set panningUpdateCallback(PanningUpdateallback callback) {
    _panningUpdateCallback = callback;
  }
  ...
  void attachTo(BaseChart<D> chart) {
    ...
    //_chart.domainAxis.setViewportSettings(initialState.scale, initialState.dx, drawAreaWidth: initialState.width);
    // setViewportSettings(initialState.scale, initialState.dx, initialState.width);
  }
  ...
  bool onDragUpdate(Point<double> localPosition, double scale) {
    ...
    setViewportSettings(
        domainScalingFactor, domainChange, chart.drawAreaBounds.width);
  ...
  void setViewportSettings(
      final double scale, final double dx, final int width) {
    // print('!!: _chart.domainAxis.viewportTranslatePx: ${_chart.domainAxis.viewportTranslatePx}');
    _chart.domainAxis.setViewportSettings(scale, dx, drawAreaWidth: width);
    //  print(
    //     'width: $width dx: $dx chart.domainAxis.viewportScalingFactor: ${chart.domainAxis.viewportScalingFactor}, _chart.domainAxis.viewportTranslatePx: ${_chart.domainAxis.viewportTranslatePx}, _isPanning: ${_isPanning}');
    if (_panningUpdateCallback != null) {
      _panningUpdateCallback(chart.domainAxis.viewportScalingFactor,
          _chart.domainAxis.viewportTranslatePx, _isPanning);
    }
  }
  ...
  void onPanStart() {
    ...
    if (_panningStartedCallback != null) {
      _panningStartedCallback();
    }
  }
  ...
  PanningStartedCallback _panningStartedCallback;
  set panningStartedCallback(PanningStartedCallback callback) {
    _panningStartedCallback = callback;
  }
...
// class CommonPanControlBehaviour<D> extends PanBehavior<D> {
//   PanningStartedCallback _panningStartedCallback;
//   set panningStartedCallback(PanningStartedCallback callback) {
//     _panningStartedCallback = callback;
//   }
...
typedef PanningStartedCallback = void Function();
typedef PanningUpdateallback = Function(
    double scale, double dx, bool isPanning);
